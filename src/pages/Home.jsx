...
const Home = () => {
  ...
  const handleSarch = (e) => {
    ...
    setSearchTimeout(
      setTimeout(() => {
        const result = allPosts.filter((image) => image.name.toLowerCase().includes(searchText.toLowerCase()) || image.prompt.toLowerCase().includes(searchText.toLowerCase()));
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        const response = await fetch('https://ai-image-generation-server.onrender.com/api/v1/post', {
          method: 'GET',
          headers: {
